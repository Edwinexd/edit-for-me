...
This file holds only instructions. Never write project state here. Keep state in `STATUS.md`: what is being edited, the clips and language, clip quirks found (e.g. "12-58-18 ends on an old outro, cut at 364.45 s"), the current review round, the user's decisions and preferences for this project, and what is signed off. Update it after every step, so that a fresh session can pick up from `STATUS.md` alone.
...
- Speech recognition (`asr.py`, set by `ASR` in `common.py`): mlx-whisper on Apple Silicon, faster-whisper (CPU or CUDA) everywhere else. Both give the same Whisper-style JSON, and the model per backend is in `MODELS`.
- Per project, set `SRC_DIR` (the clip folder) and `LANG` (Whisper language code, e.g. `"sv"`, `"en"`) in `common.py`. Clips can be any common video format (`VIDEO_EXTS`), and the review page plays whatever the browser supports. When the filenames are recording timestamps, sorted order is recording order.

## Tools
- `watch.py [--interval SECS]` transcribes each clip in `SRC_DIR` once it has finished downloading. Run it in the background and act on its READY/DONE/FAILED lines (e.g. under Monitor in Claude Code).
- `transcribe.py CLIP...` writes `work/<stem>.json` (word timestamps), `.txt` (numbered segments) and `.wav` (16 kHz mono). Clips already transcribed are skipped.
...
- `review.py` runs the review rounds (details under "Review with the user"): `publish RENDER`, `serve [--port]`, `wait [ROUND]`, `show [ROUND]`. The UI is `ui/index.html`.
